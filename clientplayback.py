...
for f in flows:
    ...
    qu.put(hf)
ctx.log.error('Fuzzing group 1 in rule ' + str(rule['value'][0]) + ' with pattern 1')
f = qu.get() 
pre_flow = f.copy()
...
def response(flow):
    ...
    if qu.empty() == False:
        f = qu.get()
        pre_flow = f.copy()
        ctx.log.error('fuzzing group ' + str(cur_group) + 
                ' in rule ' + str(rule['value'][cur_rule]) + 
                ' with pattern ' + str(cur_ptn))
        if rule['resp_rule'][cur_group] != None:
            for r in rule['resp_rule'][cur_group]:
                try:
                    #pdb.set_trace()
                    value = find_value(new[r[2] - cur_group], r[3], r[4]) 
                    f = replace_field(f, value, r[0], r[1]) 
                except Exception as e:
                    print(str(e))
        ctx.master.commands.call("test.replay", [f])
        cur_group += 1
        if cur_group > rule['group']:
            cur_group = 1 
            cur_ptn += 1
        if cur_ptn > rule['ptn_num'][cur_rule]:
            cur_ptn = 1
            cur_rule += 1
        if cur_rule == len(rule['value']):
            ctx.log.error('rule complete')
...
class test:
    ...
    def load(self, loader):
        ...
        loader.add_option("path", typing.Sequence[str], [], "")
    ...
    def load_file(self) -> None:
        ...
        """
        log_path = str(ctx.options.path)
        flows = io.read_flows_from_paths([log_path])
        flows.sort(key = lambda f: f.request.timestamp_start)     
        for f in flows:
            hf = typing.cast(http.HTTPFlow, f)
            qu.put(hf)
        ctx.master.commands.call("replay.client", [qu.get()])
    '''    
